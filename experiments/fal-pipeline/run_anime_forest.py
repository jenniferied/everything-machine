#!/usr/bin/env python3
"""
Anime Fantasy Forest Landscape — three current-gen image models at 16:9.
Models: Nano Banana 2, Seedream 4.0, Flux 2 Pro. Same prompt each.
Estimated total: ~$0.14. Re-runnable (skips completed).
"""
...
OUTPUT_DIR = SCRIPT_DIR / "outputs" / "anime_forest"
RESULTS_FILE = SCRIPT_DIR / "anime_forest_results.json"

PROMPT = (
    "Anime fantasy forest landscape, lush ancient woodland with towering trees, "
    "dappled golden sunlight filtering through dense canopy, soft glowing magical "
    "particles floating in the air, moss-covered stones and exposed roots, gentle "
    "mist drifting between trunks, a small winding path through ferns, distant "
    "glowing flowers. Studio Ghibli inspired, painterly, atmospheric depth, "
    "cinematic wide composition, vibrant but soft colors, no characters, "
    "environment only, 16:9 landscape."
)
...
MODELS = [
    ...
    },
    {
        ...
        "prompt_override": (
            ...
            "particles floating in the air, moss-covered stones and exposed roots, gentle "
            "mist drifting between trunks, a small winding path through ferns, distant "
            "glowing flowers. Anime film style, painterly, atmospheric depth, cinematic "
            "wide composition, vibrant but soft colors, no characters, environment only."
        ),
    },
]
...
def run_model(model_cfg):
    ...
    args = {"prompt": model_cfg.get("prompt_override", PROMPT)}
...
def main():
    ...
    print("=" * 60)
    print("ANIME FOREST — 3 current-gen image models at 16:9")
    if only:
